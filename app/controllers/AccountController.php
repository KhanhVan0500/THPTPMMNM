...
class AccountController {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (empty($errors)) {
                ...
                if ($account) {
                    ...
                    $this->sendAccountMail($email, 'Xác thực tài khoản KZANN',
            ...
            if ($action === 'update_profile') {
                ...
                if (empty($errors)) {
                    ...
                    if ($emailChanged && $result) {
                        ...
                        $this->sendAccountMail($email, 'Xác thực email KZANN',
            ...
            if (empty($errors)) {
                ...
                if ($account && (int)($account['is_active'] ?? 1) === 1 && !empty($account['email'])) {
                    ...
                    if ($token) {
                        ...
                        $this->sendAccountMail($account['email'], 'Đặt lại mật khẩu KZANN',
    ...
    public function resendVerification() {
        ...
        if ($account && !empty($account['email']) && empty($account['email_verified_at'])) {
            ...
            $this->sendAccountMail($account['email'], 'Gửi lại xác thực tài khoản KZANN',
    ...
    private function sendAccountMail($to, $subject, $body) {
        ...
        $headers = "From: KZANN <no-reply@kzann.vn>\r\n" .
